...
%%
pp(A, P, S) when is_atom(A) -> 
  case A of
	hier1 -> io:format("CCount in gather! ~p~n", [P]);
	hier2 -> io:format("CCount in lo! ~p ElternProz ist ~p~n", [P, S]);
	nichts -> io:format("Nichts machen ~p ElternProz ist ~p~n", [P, S])
  end.
...
%% Aufruf: mom() -> b3:tshit(fun() -> b3:FUNKTION ( ARGS ) end).
%%
mom1() -> b3:tshit(fun() -> b3:calcSquares([15,10,3,6], 4, 34) end). % 0.4 secs


%% 
%% Zu faul das immer hinzuschreiben...
%%
m1(X) -> b3:magicsquare(X). % ~10 mins
m2(X) -> b3:distribMS(X, kerne()). % ~4-5 mins mit 4 Kerne
m2(X, Y) -> b3:distribMS(X, Y).
m3(X) -> b3:megaDistribMS(X). % 5 mins 
...
%% ?MODULE ist nen Makro für den Modulnamen = b3
%%
...
loop_gather(CCount, Result) -> pp(hier1, CCount, self()),
	receive
...
%% ============================================================================
%% IST NOCH BUGGY; funktioniert nicht mehr, wenn keine Hosts mehr übrig sind
%% bel3_2 enthält komplette stabile Version, aber ohne Absturz Erkennung
%% ============================================================================
...
%%
%% im Werl Fenster von starter macht man dann: b3:megaDistribMS(4).
...
-spec while(posint(), list({atom(), posint()}), list(list(posint())), posint(),
            posint()) -> ok.
while (CCount, Hosts, PList, Max, Value) ->  
...
while(_, [{ VMName, _}], PList, Max, Value, HostCount, Len) -> 
  ...
  Argusi = [0, self(), lists:nthtail(Len * HostCount, PList), Max, Value, VMName],
  Pidi = spawn(node(), ?MODULE, init_global, Argusi),
  pDebug(Pidi, self(), node(), pstart);
...
while(CCount, [{ VMName, _} | HostsR], PList, Max, Value, HostCount, Len) -> 
  ...
  Argusa = [0, self(), lists:sublist(PList, Len * HostCount + 1, Len + Rest), Max, Value, VMName],
  Pida = spawn(node(), ?MODULE, init_global, Argusa),
  pDebug(Pida, self(), node(), pstart),
...
-spec init_global(posint(), pid(), list(list(posint())), posint(), posint(), 
  ...
  init_global(_, SPid, PList, Max, Value, Host, Try) ->
	...
	lo(PCount, [], PList, Max, Value, SPid, 0).
...
%% wenn keine Hosts mehr da sind, funktioniert es nicht :(


lo(0, Result, _, _, _, SPid, _) -> SPid ! {calc, Result}, pDebug(self(), SPid, node(), pende);		
lo(CCount, Result, Startliste, Max, Value, SPid, Flag) -> pp(hier2, CCount, SPid),
	receive
      {calc, Res} -> lo(CCount - 1, Result ++ Res, Startliste, Max, Value, SPid, Flag);
	  
	  {'EXIT', _, normal} -> % not a crash
        io:format("Normal!~n", []), lo(CCount, Result, Startliste, Max, Value, SPid, Flag);
		
      {'EXIT', _, shutdown} -> % manual shutdown, not a crash
        io:format("Shutdown!~n", []), lo(CCount, Result, Startliste, Max, Value, SPid, Flag);
		
      {'EXIT', Pid, _} ->
        io:format("~p ist abgestürzt! Wir sind in ~p in Node ~p~n ", [Pid, self(), node()]),
		case Flag of 
			0 -> host_monitor ! {getnode, self()},
					receive
					  {newhost, []} -> io:format("Keine Hosts mehr!:~n", []),
						lo(CCount - 1, Result, Startliste, Max, Value, SPid, Flag);
			
					  {newhost, Neu} -> io:format("Neu: ~p~n", [Neu]), 
						{_, PCnt} = lists:keyfind(Neu, 1, hosts()), %Neu die Zeile
						%spawn_at(1, Neu, Startliste, Max, Value, init_global),
						spawn_at(PCnt, Neu, Startliste, Max, Value, init_local),
						lo(CCount, Result, Startliste, Max, Value, SPid, 1)
			  
					  after 3000 -> io:format("Keine mehr da? ~n", []),
					    lo(0, Result, Startliste, Max, Value, SPid, Flag)
					end;
				
			1 -> pp(nichts, CCount, SPid), lo(CCount, Result, Startliste, Max, Value, SPid, Flag)
		end
    end.
