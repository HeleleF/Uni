...
BBLog.handle("add.plugin", {
    ...
    battlelog : {  
        ...
        getServerInfo: function(serverHash, callback) {
            $.ajax({
                url: 'http://battlelog.battlefield.com/bf4/servers/show/' + serverHash + "/?json=1",
                type: 'GET',
                async: true,
                cache: false,
                timeout: 30000,
    ...
    handler: function(instance) {
        ...
        $('head').append('<link rel="stylesheet" href="https://rawgit.com/HeleleF/Uni/master/BetterBattlelog%20Stuff/MoVmaP.css" type="text/css" />');
        ...
        var liveIndicator = '<div class="as-server-indicator"><div id="as-live-indicator"></div>Live</div>';

        //var roundInfoHeader = instance.drawRoundInfo(instance);
        var roundInfoHeader = '<div id="as-server-header"><div id="as-server-title">' + $(".server-title").text() + liveIndicator + '</div><div id="as-server-info"></div><div id="as-server-chart"></div></div>';
        ...
        $("#as-container").append(debugWindow);

        //Overlay DIV
        $("#as-container").append('<div class="as-overlay"></div>');
    ...
    scoreboard: {
        ...
        getTeamHeader: function(instance, scoreboardData, team) {
            ...
            var teamInfo = '<table class="as-team-summary">' +
    ...
    */
	...
	lookup : {
		...
		squadName : function(squadId) {
			var squads = ["No Squad", "Alpha", "Bravo", "Charlie", "Delta", "Echo", "Foxtrot", "Golf", "Hotel", "India", "Juliett", "Kilo", "Lima", "Mike", "Oscar", "Lima"];
